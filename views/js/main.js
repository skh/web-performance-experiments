...
// Generates the sliding pizzas on load AND resize
var createPizzas = function () {
  // check if there are already pizzas, if yes, remove
  var pizzas = document.querySelectorAll('.mover');
  var numPizzas = pizzas.length;
  if (pizzas.length > 0) {
    pizzas[i].remove();
  }

  // create new pizzas based on current window height
  var cols = 8;
  var s = 256;
...
// create pizzas when the page loads...
document.addEventListener('DOMContentLoaded', function() { 
  createPizzas();
  requestAnimationFrame(updatePositions);
});

// ... and when the page resizes
